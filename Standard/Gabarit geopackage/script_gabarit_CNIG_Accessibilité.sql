...
--date : 09/09/2025
--révision : 24/11/2025 résolution du bug enum_controleBev au lieu de enum_controle_bev (idem pour enum_masqueCovisibilite au lieu de enum_masque_covisibilite)
...
CREATE TABLE noeud_cheminement (  
  ...
  altitude REAL, 
  bandeEveilVigilance TEXT NOT NULL DEFAULT 'sans objet',
  hauteurRessaut REAL NOT NULL,
  abaissePente INTEGER NOT NULL,
  abaisseLargeur REAL NOT NULL,
  masqueCovisibilite TEXT NOT NULL DEFAULT 'aucun',
  ...
  CONSTRAINT fk_masqueCovisibilite FOREIGN KEY (masqueCovisibilite) REFERENCES enum_masque_covisibilite(valeur),
...
-- Attributs OBSTACLE
  ...
  rappelObstacle TEXT NOT NULL DEFAULT 'sans objet',
...
-- Attributs CIRCULATION
  ...
  eclairage TEXT,
  transition TEXT,	
  typePassage TEXT DEFAULT 'en surface',
  repereLineaire TEXT NOT NULL DEFAULT 'aucun',
  couvert TEXT,
  geom LINESTRING NOT NULL,
...
-- Attributs TRAVERSEE
  ...
  aideSonore TEXT NOT NULL DEFAULT 'absence',
  repereLineaire TEXT NOT NULL DEFAULT 'aucun',
  chausseeBombee BOOLEAN TEXT NOT NULL DEFAULT 'non',
  voiesTraversees TEXT DEFAULT 'VV',
  geom LINESTRING NOT NULL,
...
CREATE TABLE ascenseur (
  ...
  CONSTRAINT fk_masqueCovisibilite_ascenseur FOREIGN KEY (masqueCovisibilite) REFERENCES enum_masque_covisibilite(valeur),
...
CREATE TABLE elevateur (
  ...
  CONSTRAINT fk_masqueCovisibilite_elevateur FOREIGN KEY (masqueCovisibilite) REFERENCES enum_masque_covisibilite(valeur),
